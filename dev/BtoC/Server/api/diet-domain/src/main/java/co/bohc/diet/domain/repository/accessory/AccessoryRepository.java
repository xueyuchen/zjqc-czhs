...
public interface AccessoryRepository extends JpaRepository<Accessory, Integer> {
    ...
    @Query(
           value = "SELECT a FROM Accessory a WHERE a.saleDt IS NOT NULL AND a.model.modelName = :modelName AND a.style.styleName = :styleName AND a.part.partName = :partName")
