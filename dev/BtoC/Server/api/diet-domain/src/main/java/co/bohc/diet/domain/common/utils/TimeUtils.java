...
public final class TimeUtils {
     ...
     * @param dt
     * @param days
     * @param status 0:begin of one day.  1: end of one day
     * @return
     *
     */
    ...
    public static Date getPushDate(Date date){
        ...
        if(time > SleepTime.START.getTime() || time < SleepTime.END.getTime()){
            ...
            return new DateTime(dt.getYear(),
                    dt.getMonthOfYear(),
                    dt.getDayOfMonth(), SleepTime.END.getHour(),
                    SleepTime.END.getMinute(),
                    0).toDate();
        }
        return date;
    }
    ...
    public static String formatDate2String(Date dt) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(dt);
        return String.format(Y_M_D, cal.get(Calendar.YEAR), cal.get(Calendar.MONTH)+1, cal.get(Calendar.DAY_OF_MONTH));
    }
